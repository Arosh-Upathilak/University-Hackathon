    ...
    <div className=" fixed bg-white w-full  h-[12vh] z-30">
      <div className="flex justify-between items-center h-full w-[90%] mx-auto">
          ...
          {navBarLink.map((item, index) => {
            return (
              <Link
                key={index}
                href={item.link}
                className="cursor-pointer hover:text-black/50 transition-all duration-200"
              >
                {item.name}
              </Link>
            );
        ...
        <div className="md:hidden text-black cursor-pointer">
          {!showMobileNavBar&&<HiBars3BottomRight className="w-10 h-10" onClick={()=>setShowMobileNavBar(true)}/>}
